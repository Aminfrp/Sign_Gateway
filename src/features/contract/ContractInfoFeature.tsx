    ...
    const handleRejectContract = async () => {
        try {
            ...
            if(rejectData.status === 200){
                ...
                window.location.href = contract?.result.unsuccessfulDeepLink as string;
            }
        }catch (e) {
            console.log(e)
        }
    ...
    const onSubmit = async (value: yup.InferType<typeof schema>) => {
        try {
            ...
            if (
                ...
                (lastIqueryStatus?.status === USER_LEVEL_STATUS.SABTE_AHVAL_OK && contract?.result.faceVerificationForced)
            ) {
                ...
                >
                  ...
                  >
                    ...
                    <Button
                        ...
                        type="submit"
                    >
                        {contract?.result.faceVerificationForced? "احراز هویت ویدویی" :  "اعتبارسنجی و امضای سند"}
                    </Button>
                </form>
            </Card>
            <ContractSignStatus/>
