...
export const services = {
  getContract: async (data: { sign: string }) =>
    await api
      .get<ContractResponseFromLink>(
        `/api/gateway/load/v2?${convertJsonToQueryString(data)}`
      )
      .then((res) => res.data),
  ...
  signContract: async (payload: {
    ...
    ...(payload.trackerId ? { faceVerificationTracer: payload.trackerId } : {}),
    }
    const options = {
      headers: {
    ...
    return await api.post(`/api/gateway/sign/v2`, data, options);
  },
  ...
  rejectContract: async (payload:string) => await api.post("/api/gateway/reject/v2",{signature:payload})
};
